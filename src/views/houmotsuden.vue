...
const adultCount = ref('7');
const childCount = ref('11');
  ...
  <DashboardLayout :onSave="handleSave" :onPrint="handlePrint">
    ...
    <template #inputForm>
      <div class="flex flex-wrap items-center gap-6">
        ...
        <!-- 割引人数（大人） -->
        <div class="flex items-center gap-2">
          <span class="font-bold text-gray-700">割引人数（大人）</span>
          <input
            type="text"
            v-model="adultCount"
            class="bg-white border border-gray-400 rounded px-3 py-1 text-right w-20 shadow-inner font-sans"
          />
        </div>

        <!-- 割引人数（小人） -->
        <div class="flex items-center gap-2">
          <span class="font-bold text-gray-700">割引人数（小人）</span>
          <input
            type="text"
            v-model="childCount"
            class="bg-white border border-gray-400 rounded px-3 py-1 text-right w-20 shadow-inner font-sans"
      ...
      <table class="w-full border-collapse text-sm 2xl:text-base">
        <thead>
          <tr class="border-b border-gray-400 bg-gray-100/70 font-bold text-gray-800">
            ...
            <th class="p-3.5 border-r border-gray-300 w-[25%] text-center">金額</th>
            <th class="p-3.5 border-r border-gray-300 w-[27.5%] text-center">
              割引人数（大人）
            </th>
            <th class="p-3.5 text-center w-[27.5%]">割引人数（小人）</th>
          </tr>
        </thead>
        <tbody>
          <tr
