...
const SIDE_MENU = [
  ...
  { id: '授印', label: '授印', icon: '🖊️', href: '#', active: true },
...
export default function JyuinPage() {
  const [inputDate, setInputDate] = useState('2026/07/30');
  const [sheets, setSheets] = useState('78');
  ...
  const handleAdd = () => {
    if (!sheets) return;
    setDataList([...dataList, { date: inputDate, sheets, amount: `${amount} 円` }]);
  };
      ...
      <div className="flex-1 flex flex-col h-screen min-w-0">
        <header className="bg-gray-200/90 backdrop-blur-sm border-b border-gray-300 shrink-0 sticky top-0 z-10 p-6 2xl:p-8 space-y-4 shadow-sm">
          <div className="flex justify-between items-center">
            <h1 className="text-xl font-bold tracking-wider text-slate-800">授印</h1>
          ...
          <div className="bg-[#FFEAD0] p-4 2xl:p-5 rounded-xl shadow-md border border-[#FCD29F] flex flex-wrap items-center justify-between gap-4 text-sm 2xl:text-base">
            <div className="flex flex-wrap items-center gap-6">
              ...
              <div className="flex items-center gap-2">
                <span className="font-bold text-gray-700">枚数</span>
                <input type="text" value={sheets} onChange={(e) => setSheets(e.target.value)} className="bg-white border border-gray-400 rounded px-3 py-1 text-right w-24 shadow-inner font-sans" />
              </div>
              <div className="flex items-center gap-2">
            ...
            <table className="w-full border-collapse text-sm 2xl:text-base">
              <thead>
                <tr className="border-b border-gray-400 bg-gray-100/70 font-bold text-gray-800">
                  <th className="p-3.5 border-r border-gray-300 w-[30%] text-center">日付</th>
                  <th className="p-3.5 border-r border-gray-300 w-[30%] text-center">枚数</th>
                  <th className="p-3.5 text-center w-[40%]">金額</th>
                </tr>
              </thead>
              <tbody>
                {dataList.map((row, index) => (
                  <tr key={index} className="border-b border-gray-300 hover:bg-slate-50">
                    ...
                    <td className="p-3 text-right border-r border-gray-300 font-sans tabular-nums pr-12">{row.sheets}</td>
                    <td className="p-3 text-right font-sans tabular-nums pr-12">{row.amount}</td>
                  </tr>
                ))}
              </tbody>
            </table>
