...
const SIDE_MENU = [
  ...
  { id: '賽銭', label: '賽銭', icon: '🪙', href: '/saisen', active: true },
...
// テストデータ（DB疎通時に削除）
const INITIAL_DATA = [
  { date: '2026/07/30', code: '010 - 010', name: '祈祷所', amount: '123,456 円' },
  { date: '2026/07/30', code: '010 - 010', name: '上宮授与所', amount: '123,456 円' },
  { date: '2026/07/30', code: '010 - 010', name: '楼内授与所', amount: '123,456 円' },
  { date: '2026/07/30', code: '010 - 010', name: '丸山授与所', amount: '123,456 円' },
  { date: '2026/07/30', code: '010 - 010', name: '東授与所', amount: '123,456 円' },
  { date: '2026/07/30', code: '010 - 010', name: '西授与所', amount: '123,456 円' },
  { date: '2026/07/30', code: '010 - 010', name: '東みくじ所', amount: '123,456 円' },
];

export default function SaisenOrOmikujiPage({ pageTitle = '賽銭' }: { pageTitle?: string }) {
  const [date, setDate] = useState(DEFAULT_DATE);
  const [placeCode, setPlaceCode] = useState('010-010');
  const [placeName] = useState('祈祷所');
  const [amount, setAmount] = useState('234,567');
  const [dataList, setDataList] = useState(INITIAL_DATA);

  const handleAdd = () => {
    setDataList([...dataList, { date: date, code: placeCode, name: placeName, amount: `${amount} 円` }]);
  };
            ...
            <table className="w-full border-collapse text-sm 2xl:text-base">
              <thead>
                <tr className="border-b border-gray-400 bg-gray-100/70 font-bold text-gray-800">
                  ...
                  <th className="p-3.5 border-r border-gray-300 w-[20%] text-center">場所コード</th>
