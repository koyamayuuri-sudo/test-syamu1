...
const SIDE_MENU = [
  ...
  { id: '参拝施設所', label: '参拝施設所', icon: '☕', href: '/sanpai', active: true },
...
export default function SanpaiShisetsuPage() {
  ...
  const handleAdd = () => {
    setDataList([...dataList, { date: date, code: placeCode, name: placeName, method: paymentMethod, amount: `${amount} 円` }]);
  };
    ...
    <div className="flex h-screen bg-[#FDFBF7] font-sans text-gray-800 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-screen min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-y-3 gap-x-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="relative">
                  <select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} className="bg-white border border-gray-400 rounded px-4 py-1 pr-8 appearance-none shadow-inner font-sans">
        ...
        <main className="flex-1 p-6 2xl:p-8 flex flex-col justify-between overflow-y-auto">
              ...
              <tbody>
                {dataList.map((row, index) => (
                  <tr key={index} className="border-b border-gray-300 hover:bg-slate-50">
                    ...
                    <td className="p-3 text-center border-r border-gray-300 font-sans">{row.method}</td>
          ...
          <div className="flex justify-end mt-6 flex-col sm:flex-row gap-4">
            <button className="bg-[#FF4D4D] hover:bg-[#E63939] text-white px-10 py-2 rounded-md font-bold shadow transition active:translate-y-0.5 w-full sm:w-auto">
              保存
            </button>
            <button className="bg-[#997A00] hover:bg-[#806600] text-white px-10 py-2 rounded-md font-bold shadow transition active:translate-y-0.5 w-full sm:w-auto">
              帳票出力
            </button>
          </div>
        </main>
