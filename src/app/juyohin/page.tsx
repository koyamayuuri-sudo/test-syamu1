...
const SIDE_MENU = [
  ...
  { id: '授与品', label: '授与品', icon: '🏷️', href: '/juyohin', active: true },
...
export default function JyuyohinPage() {
  const [date, setDate] = useState(DEFAULT_DATE);
  const [itemCode, setItemCode] = useState('78');
  const [itemName, setItemName] = useState('品目名称');
  ...
  const handleAdd = () => {
    ...
    setDataList([
      ...dataList,
      { date: date, code: itemCode, name: itemName, count, amount: `${amount} 円` },
    ]);
  };
      ...
      <div className="flex-1 flex flex-col h-screen min-w-0">
        <header className="bg-gray-200/90 backdrop-blur-sm border-b border-gray-300 shrink-0 sticky top-0 z-10 p-6 2xl:p-8 space-y-4 shadow-sm">
          <div className="flex justify-between items-center">
            <h1 className="text-xl font-bold tracking-wider text-slate-800">授与品</h1>
          ...
          <div className="bg-[#FFEAD0] p-4 2xl:p-5 rounded-xl shadow-md border border-[#FCD29F] flex flex-col gap-4 text-sm 2xl:text-base">
            ...
            <div className="flex flex-wrap items-center gap-y-3 gap-x-6">
              ...
              {/* 品目コード・名称 */}
              <div className="flex items-center gap-2">
                <span className="font-bold text-gray-700">品目コード</span>
                <div className="relative">
                  <select value={itemCode} onChange={(e) => setItemCode(e.target.value)} className="bg-white border border-gray-400 rounded px-4 py-1 pr-8 text-center appearance-none shadow-inner font-sans">
                    <option value="78">78</option>
                  </select>
                  <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs pointer-events-none text-gray-600">▼</span>
                </div>
                <input type="text" readOnly value={itemName} className="bg-gray-200 border border-gray-400 rounded px-3 py-1 w-48 text-gray-700 shadow-inner" />
              </div>
              {/* 数量 */}
              <div className="flex items-center gap-2">
                <span className="font-bold text-gray-700">数量</span>
                <input type="text" value={count} onChange={(e) => setCount(e.target.value)} className="bg-white border border-gray-400 rounded px-3 py-1 text-right w-24 shadow-inner font-sans" />
              </div>
              {/* 金額 */}
              <div className="flex items-center gap-2">
            ...
            <table className="w-full border-collapse text-sm 2xl:text-base">
              <thead>
                <tr className="border-b border-gray-400 bg-gray-100/70 font-bold text-gray-800">
                  ...
                  <th className="p-3.5 border-r border-gray-300 w-[15%] text-center">品目コード</th>
                  <th className="p-3.5 border-r border-gray-300 w-[30%] text-center">名称</th>
                  <th className="p-3.5 border-r border-gray-300 w-[15%] text-center">数量</th>
              ...
              <tbody>
                {dataList.map((row, index) => (
                  <tr key={index} className="border-b border-gray-300 hover:bg-slate-50">
                    ...
                    <td className="p-3 text-right border-r border-gray-300 font-sans tabular-nums pr-8">{row.count}</td>
